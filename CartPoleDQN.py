...
gamma = 1.0 					# Used to calculate discounted reward
epsilon = 1.0					# Exploration (Randomly chooses action for the best long term affect)
epsilon_min = 0.01				# Lowest value the agent can explore
epislon_decay = 0.995 			# Used to lower the epsilon value as the agent gets better at playing

number_episodes = 6000 	# Total episodes to play
memory = deque(maxlen = 60000)	# Stores all experiances
...
def get_epsilon(episode): # Lowers the epsilon based on amount of episodes
	return max(epsilon_min, min(epsilon, 1.0 - math.log10((episode + 1) * epislon_decay))) 


def replay(batch_size, epsilon, model):
	if len(memory) < batch_size: # replay again until batch_size is large enough
		return

	mini_batch = random.sample(memory, batch_size) # Sample some experiances from memory (previous experiances)
	for state, action, reward, next_state, done in mini_batch: # Extracting information from each memory
		...
		if not done: # Predict the future discounted reward
			...
			q_update = reward + gamma * np.amax(model.predict(next_state)[0])

		q_value = model.predict(state) # Predict the reward of the current state based on the data we have trained with
		q_value[0][action] = q_update

		# Train the neural network with the state and Q value
...
def run():
	...
	win_goal = 195			# Average of 100 episodes must be higher than 195
	...
	for eps in range(number_episodes):
		...
		done = False # Resetting done to false as a new game has started
		i = 0 # Reset score
		...
		if done: # Episode is over
			...
			if (average_score > 200):
				...
				model.save("trained_DQN_model_average300.p5")
	...
	if eps is number_episodes and average_score < win_goal: # If we have reached the desired score after all games

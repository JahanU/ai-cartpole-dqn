...
def load_CNN(env): 
	...
	# model = load_model("trained_DQN.p5") 			   # Trained model which can average 195
	model = load_model("trained_DQN_average_400.p5") # Trained model which can average 400 
	...
	for eps in range(number_episodes):
		...
		if done: # Episode is over
			...
			if (average_score > win_goal):
				print("Reached the average target of: {}. In {} episodes. Average score was: {}. ".format(win_goal, eps, average_score))
